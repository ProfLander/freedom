...
- Reintroduce threading
  - [x] Expose means for scheme to spawn threads
  - [ ] Implement local equivalent of SerializableSteelVal, minus the illegal-to-share parts
    - Necessary for access, as built-in inter-thread pipeline crashes without `sync` feature
  - [ ] Implement bindings to smol channels
    - Provide blocking / async / try methods for convenience
  - [ ] Implement spawn-thread-with-environment setup
    - Pass quoted entrypoint (list of exprs, not a lambda) and environment as serializable
    - Unserialize on the other end and evaluate
      - NOTE: Can likely avoid existing convert-to-string-and-run pattern
        by registering quoted value with engine as `*meta-expression*` or similar

- [ ] Restructure script system around scheme modules
  - `module` syntax yields a hash table of provided values, binds them in the defining scope
  - i.e. `(module my-module (provide gubbin) (define gubbin 1234))` yields
         `'#hash((gubbin . 1234))`
  - Provides surface to implement a system akin to Lua modules
  - Avoids having to store scripts as `RawProgramWithSymbols`
    - Can wrap textual scripts in `(module <script name> . <body>)`
      - `provide` form is mandatory, so will need to check body for it and sub in `(provide void)` if missing
  - Should ultimately be able to replace the built-in `require` with a script-addressing version
    - Use identifiers for paths, no extension, relative to script folder root
      - i.e. Allow local paths via `child/module`, and rooted via `/child/module`

- [ ] Fix MAV when dropping active file watcher
  - Occurs due to `notify-windows` thread not terminating immediately on drop
    - Attempts to deallocate, and fails
  - Need to check if the underlying watcher has an interface to this
